...
# If the model doesn't exist, it crashed, likely due to OOM. If so, we retry with a smaller number of blocks.
if [[ ! -d $model_path'/model.safetensors' ]]; then

        exp_name="64_tokens-464_blocks"
        model_path=$STORAGE_DIR'/models/longtriever_og/'$exp_name
        echo $model_path
        if [[ ! -d $model_path ]]; then
        mkdir -p $model_path
