...
class TripletDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, i):
        ...
        return {"queries":query, "positives":positives, "negatives":negatives}
...
def get_triplets_dataloader(batch_size, dataset_path, **kwargs):
    effective_batch_size = int(batch_size / 2)
    dataset = TripletDataset(torch.load(dataset_path, weights_only=True))
    dataloader = torch.utils.data.DataLoader(dataset, batch_size = effective_batch_size, shuffle=True, collate_fn=TripletDataset.custom_collate_fn, **kwargs)
