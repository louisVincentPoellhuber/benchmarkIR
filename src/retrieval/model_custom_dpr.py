...
class DPREncoder(DPRPreTrainedModel):
    ...
    def __init__(self, config: DPRConfig):
        ...
        if self.max_position_embeddings > 0:
            self.encode_proj = nn.Linear(self.roberta_model.config.hidden_size, config.max_position_embeddings)
    ...
    def embeddings_size(self) -> int:
        ...
        return self.roberta_model.config.hidden_size
...
class CustomDPR:
    def __init__(self, config: CustomRobertaConfig, model_path: Union[str, Tuple] = None, device: str = "cuda:0", **kwargs):
        self.config = config
        self.device = device
    ...
    def save(self, save_path):
        ...
        if not os.path.exists(ctx_path): os.mkdir(ctx_path)
