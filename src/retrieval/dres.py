...
class DenseRetrievalExactSearch(BaseSearch):
    ...
    def search(self, 
        ...
        #query_itr = range(0, len(queries), self.batch_size)
        
        #for batch_num, query_start_idx in enumerate(query_itr):
        #    if batch_num==1:
        #        query_embeddings = sub_query_embeddings
        #    query_end_idx = min(query_start_idx + self.batch_size, len(queries))

        #    sub_query_embeddings = self.model.encode_queries(
        #        queries[query_start_idx: query_end_idx], 
        ...
        #        )
            ...
            #if batch_num>0:
            #    query_embeddings = torch.cat((query_embeddings, sub_query_embeddings))
