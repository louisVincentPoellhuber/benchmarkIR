...
class RobertaEmbeddings(nn.Module):
    ...
    def forward(
        ...
        #print(f"Position embedding type: {self.position_embedding_type}")
...
# Copied from transformers.models.bert.modeling_bert.BertSelfAttention with Bert->Roberta
class RobertaSparseSelfAttention(nn.Module):
    def __init__(self, config, position_embedding_type=None):
        super().__init__()
        if config.hidden_size % config.num_attention_heads != 0 and not hasattr(config, "embedding_size"):
            raise ValueError(
    ...
    def forward(
        ...
        # Normalize the attention scores to probabilities.
        attention_probs = entmax_bisect(attention_scores, alpha=alpha, n_iter=24)
...
# Copied from transformers.models.bert.modeling_bert.BertSelfAttention with Bert->Roberta
class RobertaAdaptiveSelfAttention(nn.Module):
    def __init__(self, config, position_embedding_type=None):
        super().__init__()
        if config.hidden_size % config.num_attention_heads != 0 and not hasattr(config, "embedding_size"):
            raise ValueError(
    ...
    def forward(
        self,
        hidden_states: torch.Tensor, # 16x512x768
        attention_mask: Optional[torch.FloatTensor] = None, # None
        head_mask: Optional[torch.FloatTensor] = None, # None
        encoder_hidden_states: Optional[torch.FloatTensor] = None, # None
        encoder_attention_mask: Optional[torch.FloatTensor] = None, # None
        past_key_value: Optional[Tuple[Tuple[torch.FloatTensor]]] = None, # None
        ...
        if is_cross_attention and past_key_value is not None: # Nope
            # reuse k,v, cross_attentions
            key_layer = past_key_value[0]
            value_layer = past_key_value[1]
            attention_mask = encoder_attention_mask
        elif is_cross_attention: # Also no
            key_layer = self.transpose_for_scores(self.key(encoder_hidden_states))
            value_layer = self.transpose_for_scores(self.value(encoder_hidden_states))
            attention_mask = encoder_attention_mask
        elif past_key_value is not None: # No for the first iteration at least
            key_layer = self.transpose_for_scores(self.key(hidden_states))
            value_layer = self.transpose_for_scores(self.value(hidden_states))
            key_layer = torch.cat([past_key_value[0], key_layer], dim=2)
            value_layer = torch.cat([past_key_value[1], value_layer], dim=2)
       ...
       # key_layer, value_layer, temp = self.adaptive_mask.trim_memory(query_layer, key_layer, value_layer, None)
        ...
        #print(f"Final output: {context_layer.size()}")

        outputs = (context_layer,)

        if self.is_decoder: # Still not a decoder
            outputs = outputs + (past_key_value,)
        return outputs
...
ROBERTA_SELF_ATTENTION_CLASSES = {
    ...
    "sparse": RobertaSparseSelfAttention,
    "adaptive": RobertaAdaptiveSelfAttention,
}
...
class RobertaAttention(nn.Module):
    def __init__(self, config, position_embedding_type=None):
        super().__init__()
        self.self = ROBERTA_SELF_ATTENTION_CLASSES[config.attn_mechanism](
...
class RobertaLayer(nn.Module):
    ...
    def forward(
        self,
        hidden_states: torch.Tensor, # 16x512x768
        attention_mask: Optional[torch.FloatTensor] = None, # 16x1x1x512
        head_mask: Optional[torch.FloatTensor] = None, # None
        encoder_hidden_states: Optional[torch.FloatTensor] = None, # None
        encoder_attention_mask: Optional[torch.FloatTensor] = None, # None
        past_key_value: Optional[Tuple[Tuple[torch.FloatTensor]]] = None, # None
        ...
        self_attn_past_key_value = past_key_value[:2] if past_key_value is not None else None # So None
...
class RobertaEncoder(nn.Module):
    ...
    def forward(
        self,
        hidden_states: torch.Tensor, # tensor, 16x512x768
        attention_mask: Optional[torch.FloatTensor] = None, # tensor, 16x1x1x512
        head_mask: Optional[torch.FloatTensor] = None, # List of 6 Nones
        encoder_hidden_states: Optional[torch.FloatTensor] = None, # None at first
        encoder_attention_mask: Optional[torch.FloatTensor] = None, # None at first
        past_key_values: Optional[Tuple[Tuple[torch.FloatTensor]]] = None, # None at first
        ...
        if self.gradient_checkpointing and self.training: # Does not enter
            if use_cache:
                logger.warning_once(
        ...
        for i, layer_module in enumerate(self.layer): # Enters layers
            if output_hidden_states: # Does not enter
                all_hidden_states = all_hidden_states + (hidden_states,)

            layer_head_mask = head_mask[i] if head_mask is not None else None # So None
            past_key_value = past_key_values[i] if past_key_values is not None else None # Same

            if self.gradient_checkpointing and self.training: # No gradient checkpointing
            ...
            else: # Yes, we enter here
                layer_outputs = layer_module(
                    hidden_states, # 16x512x768
                    attention_mask, # 16x1x1x512
...
class CustomRobertaModel(RobertaPreTrainedModel):
    ...
    def forward(
        ...
        #print("\nRobertaModel, before the embeddings.")
        #print(f"Input IDs: {input_ids.shape}")
        #print(f"Token Type IDs: {token_type_ids.shape}")
        embedding_output = self.embeddings(
            input_ids=input_ids, # tensor, 16x512
        ...
        #print("\nBefore the encoder.")
        #print(f"Embedding output: {embedding_output.shape}")
        #print(f"extended_attention_mask {extended_attention_mask.shape}")
        encoder_outputs = self.encoder(
            embedding_output, # tensor, 16x512x768
            attention_mask=extended_attention_mask, # tensor, 16x1x1x512
            head_mask=head_mask, # List of 6 Nones
...
class RobertaForSequenceClassification(RobertaPreTrainedModel):
    ...
    def forward(
        ...
        if self.config.attn_mechanism == "adaptive":
            if self.config.adapt_span_enabled:
                loss += sum(layer.attention.self.adaptive_mask.get_loss() for layer in self.roberta.encoder.layer)
